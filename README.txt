...
После запуска каждый модуль содержит 3 потока. Родительский поток запускает потоки работы с рабочим объектом и каналами связи,
после чего переходит в ожидание завершения дочерних потоков.
...
Каждый поток имеет свою целевую функцию self.work_with_pipe() и self.work_with_object ().
...
При получении задачи от другого модуля проверяется принадлежит ли задача к списку стандартных задач, если принадлежит,
