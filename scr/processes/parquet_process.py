...
class ParquetProcess(BaseProcess):
    ...
    def action(self):
        ...
        while self.b_work:
            self.to_main_pipe_worker.work(timeout=self.parquet_worker.config.PIPE_TIMEOUT)
            self.to_ui_pipe_worker.work(timeout=self.parquet_worker.config.PIPE_TIMEOUT)
            self.to_camera_pipe_worker.work(timeout=self.parquet_worker.config.PIPE_TIMEOUT)
    ...
    def from_main_task_handler(self, task):
        ...
        if name == 'next task':
            ...
            self.create_logging_task(data='Parquet process task from main the solution is not defined')
    ...
    def from_ui_task_handler(self, task):
        ...
        if name == 'next task':
            ...
            self.create_logging_task(data='Parquet process task from ui the solution is not defined')
    ...
    def from_camera_task_handler(self, task):
        ...
        if name == 'Write to parquet':
            ...
            self.create_logging_task(data='Parquet process task from camera the solution is not defined')
